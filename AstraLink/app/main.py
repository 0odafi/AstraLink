...
def public_profile_page(
    ...
    open_app_url = html.escape(f"astralink://u/{user.username}")
...
<html lang="en">
  <head>
    ...
    <title>{display_name} on AstraLink</title>
    <meta name="description" content="{display_name} {handle} on AstraLink" />
    <meta property="og:title" content="{display_name} on AstraLink" />
  ...
  <body>
    <main class="card">
      ...
      <div class="actions">
        <a class="primary" href="{open_app_url}">Open in AstraLink</a>
        <button class="secondary" type="button" onclick="navigator.clipboard.writeText('{page_url}')">Copy link</button>
      </div>
      <div class="note">If the app is not installed, copy {handle} or this link and open a private chat from AstraLink.</div>
    </main>
  </body>
</html>"""
