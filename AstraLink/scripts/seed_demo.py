...
def run() -> None:
    ...
    db = SessionLocal()
    try:
        alice = _get_or_create_user(db, "alice", "alice@astralink.dev", "Password123")
        bob = _get_or_create_user(db, "bob", "bob@astralink.dev", "Password123")
        carol = _get_or_create_user(db, "carol", "carol@astralink.dev", "Password123")
        ...
        create_message(db, chat.id, alice.id, "Welcome to AstraLink demo space.")
        ...
        create_post(
            ...
            author_id=alice.id,
            payload=PostCreate(
                content="AstraLink beta is now running locally.",
                visibility="public",
            ),
        )
        create_post(
